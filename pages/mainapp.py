...
def display_available_buttons(available_buttons, search_terms, is_multi=False):
    ...
    if len(available_buttons) >= 2:
        if st.button("Comprehensive Search", icon=":material/search:", use_container_width=True, key="btn_comprehensive_search"):
            st.session_state["programmatic_nav"] = True
            st.session_state["current_page"] = "SEARCH"
            st.rerun()
    
    icon_mapping = {
        'Sequence Data': ':material/database:',
        'Biochemical Properties': ':material/grain:',
        'Protein PPI': ':material/shape_line:',
        'Cellular Localization': ':material/pin_drop:',
        'GO & KEGG': ':material/bubble_chart:',
        'FPKM Matrix': ':material/biotech:',
        'Expression Atlas': ':material/biotech:',
        'mRNA Data': ':material/ballot:',
        'lncRNA Data': ':material/ballot:',
        'miRNA Targets': ':material/merge_type:',
        'Transcription Factors': ':material/account_tree:',
        'Orthologs': ':material/compare_arrows:',
        'Paralogs': ':material/compare_arrows:',
    }

    for i, button_label in enumerate(available_buttons):
        icon = icon_mapping.get(button_label, ':material:help-circle')
        button_key = f"btn_{button_label}"
        if st.button(button_label, icon=icon, use_container_width=True, key=button_key, type="secondary"):
...
def home_page():        
    ...
    with col1:
        ...
        with con:
            if st.button("Browse", icon=":material/list_alt:", use_container_width=True, key="navBrowse", type="primary"):
                st.session_state["programmatic_nav"] = True
        ...
        with con:
            ...
            #    st.rerun()
            if st.button("BLAST", icon=":material/genetics:", use_container_width=True, key="navBlast", type="primary"):
                st.session_state["programmatic_nav"] = True
                st.session_state["current_page"] = "BLAST"
                st.rerun()
        con=st.container(border=False, key="con103hp")
        with con:
            if st.button("SNP Calling", icon=":material/content_paste_search:", use_container_width=True, key="navSNP", type="primary"):
                st.session_state["programmatic_nav"] = True
    ...
    with col3:
        ...
        with con:
            if st.button("Query Search",icon=":material/database_search:", use_container_width=True, key="navQuery", type="primary"):
                st.session_state["programmatic_nav"] = True
                st.session_state["current_page"] = "QUERY"
                st.rerun()
        con=st.container(border=False, key="con05hp")
        with con:
            if st.button("Gene Structure", icon=":material/data_table:", use_container_width=True, key="navGSDS", type="primary"):
                st.session_state["programmatic_nav"] = True
        ...
        with con:
            ...
            #    st.rerun()
            if st.button("Tools", icon=":material/design_services:", use_container_width=True, key="navTools", type="primary"):
                st.session_state["programmatic_nav"] = True
                st.session_state["current_page"] = "TOOLS"
                st.rerun()
    st.markdown("""
    <style>
    /* Icon and Text Container Styling - treat icon and text as a group */
[data-testid="stBaseButton-primary"] {
    display: flex !important;
    flex-direction: column !important;  /* Stack icon above text */
    align-items: center !important;  /* Center horizontally */
    justify-content: center !important;  /* Center vertically */
    min-height: 160px !important;  /* Minimum height of the button */
    ...
    height: 100% !important;  /* Ensure the button takes up full height of its container */
}
...
[data-testid="stBaseButton-primary"] span[data-testid="stIconMaterial"] {
    ...
    margin-bottom: 5px !important;  /* Space between icon and text for stacked layout */
}

/* Styling for button text */
[data-testid="stBaseButton-primary"] div[data-testid="stMarkdownContainer"] p {
    font-size: 2.5rem !important;  /* Font size for button text */
    margin: 0 !important;
    line-height: 1.2 !important;  /* Ensure line height for wrapping */
...
[data-testid="stBaseButton-primary"] span[data-testid="stIconMaterial"]:hover {
    color: white !important;  /* Icon color changes to white on hover */
}

/* Ensures the containers (like con101hp, con102hp) are consistent */
...
def search_page():
    ...
    if st.session_state.get("programmatic_nav", False):
        ...
        if c1.button("← Back to Home", key="back_to_home_spatial", type="primary"):
